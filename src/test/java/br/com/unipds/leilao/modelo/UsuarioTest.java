...
public class UsuarioTest extends JPATest {
	...
	public void alteraUsuario(){
		...
		Assertions.assertEquals(1, alterado);
		Assertions.assertEquals(0, original);
	}
	...
	public void armazenaEnderecoDoUsuario(){
		...
		Assertions.assertEquals("SCS1", enderecoAdicionado.getLogradouro());
		Assertions.assertEquals("Asa Sul", enderecoAdicionado.getBairro());
		Assertions.assertEquals("Brasília", enderecoAdicionado.getCidade());
		Assertions.assertEquals("DF", enderecoAdicionado.getEstado());
		Assertions.assertEquals("12345-678", enderecoAdicionado.getCep());
	}
	...
	public void adicionaFormasDePagamentoDiversas(){
		...
		Assertions.assertEquals(2, formasAdicionadas.size());
		
		Assertions.assertTrue(formasAdicionadas.stream().anyMatch(forma -> forma instanceof CartaoDeCredito));
		Assertions.assertTrue(formasAdicionadas.stream().anyMatch(forma -> forma instanceof ContaBancaria));
